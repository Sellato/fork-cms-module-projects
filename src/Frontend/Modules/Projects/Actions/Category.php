...
class Category extends FrontendBaseBlock
{
    ...
    {
        // create the form
        $this->frm = new FrontendForm('projectsIndexForm', null, 'get', null, false);
        $categories = FrontendProjectsCategoriesModel::getForMultiCheckboxForParent($this->record['id']);
        if (!empty($categories)) {
    ...
    private function getData()
    {
        ...
        $this->record = FrontendProjectsCategoriesModel::get($parameter);
        ...
        if ($this->filter['form'] == null) {
            $children = FrontendProjectsCategoriesModel::getAllChildrenByPath($this->record['path']);
        ...
        // set URL and limit
        $this->pagination['url'] = Navigation::getURLForBlock('Projects') . '?' . http_build_query($this->filter);
        $this->pagination['limit'] = $this->get('fork.settings')->get('Projects', 'overview_num_items', 10);
        $this->pagination['limit'] = 100;

        // populate count fields in pagination
        $this->pagination['num_items'] = FrontendProjectsModel::getAllCount($this->filter);
        ...
        // get articles
        $this->items = FrontendProjectsModel::getAll($this->pagination['limit'], $this->pagination['offset'], $this->filter);
    }
    ...
    private function parse()
    {
        ...
        $allCategories = FrontendProjectsCategoriesModel::getAll(array('parent_id' => 0));
        unset($allCategories[$this->record['id']]);
        $this->tpl->assign('widgetProjectsCategories', $allCategories);
