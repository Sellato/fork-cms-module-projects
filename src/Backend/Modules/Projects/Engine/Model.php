...
class Model
{
    ...
    {
        BackendModel::get('database')->delete('projects', 'id = ?', (int) $id);
        BackendModel::get('database')->delete('project_content', 'project_id = ?', (int) $id);
        BackendModel::get('database')->delete('projects_linked_catgories', 'project_id = ?', (int) $id);

        $images = (array) BackendProjectsImagesModel::getAll((int) $id);
        foreach($images as $image){
            BackendModel::deleteThumbnails(FRONTEND_FILES_PATH . '/' . BackendModel::get('url')->getModule() . '/uploaded_images',  $image['filename']);
        }

        BackendModel::get('database')->execute('DELETE c FROM project_images_content c INNER JOIN project_images i ON c.image_id = i.id WHERE i.project_id = ?', array((int) $id));
        BackendModel::get('database')->delete('project_images', 'project_id = ?', (int) $id);
    }
    ...
    public static function get($id)
    {
        ...
        $return['content'] = (array) $db->getRecords(
            'SELECT i.* FROM project_content AS i
            WHERE i.project_id = ?',
    ...
    public static function insert(array $item)
    {
        ...
        return (int) BackendModel::get('database')->insert('projects', $item);
    }
    ...
    public static function insertContent(array $content)
    {
        foreach($content as &$item){
            ...
            if( BackendModel::get('fork.settings')->get('Projects', 'make_widget_per_project') == true)
            {
                $data = [
                    'id' => $item['project_id'],
                    'language' => $item['language'],
                    'extra_label' => 'Project: ' . $item['name'],
                ];
            ...
            BackendModel::get('database')->insert('project_content', $item);
        }
    }
    ...
    public static function updateContent(array $content, $id)
    {
        ...
        {
            if( BackendModel::get('fork.settings')->get('Projects', 'make_widget_per_project') == true && $row['extra_id'])
            {
                $data = [
                    'id' => $row['project_id'],
                    'language' => $row['language'],
                    'extra_label' => 'Project: ' . $row['name'],
                ];

                BackendModel::updateExtra($row['extra_id'], 'data', $data);
            }

            $db->update('project_content', $row, 'project_id = ? AND language = ?', array($id, $language));
        }
    }
}
